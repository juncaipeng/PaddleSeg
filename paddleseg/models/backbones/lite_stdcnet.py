...
from paddleseg.models.layers.layer_libs import SyncBatchNorm, SeparableConvBNReLU

__all__ = ["LiteSTDC1", "LiteSTDC2"]


class LiteSTDCNet(nn.Layer):
    """
    The LiteSTDCNet implementation based on PaddlePaddle. The LiteSTDCNet is designed for mobile and edge devices.
    ...
    Args:
        ...
        layers(list, optional): layers numbers list. It determines STDC block numbers of LiteSTDCNet's stage3\4\5. Defualt: [4, 5, 3].
    ...
    def __init__(self,
                 ...
                 type="cat",
                 pretrained=None):
        super(LiteSTDCNet, self).__init__()
        if type == "cat":
            block = CatBottleneck
        elif type == "add":
            block = AddBottleneck
    ...
    def _make_layers(self, base, layers, block_num, block):
        ...
        features += [SeparableConvBNReLU(base // 2, base, 3, 2)]
    ...
    def __init__(self, in_planes, out_planes, block_num=3, stride=1):
        ...
        for idx in range(block_num):
            if idx == 0:
                ...
                self.conv_list.append(
                    SeparableConvBNReLU(out_planes // 2, out_planes // 2, 3))
            elif idx == 1 and block_num > 2:
                self.conv_list.append(
                    SeparableConvBNReLU(out_planes // 2, out_planes // 4, 3))
            elif idx < block_num - 1:
                self.conv_list.append(
                    SeparableConvBNReLU(out_planes // int(math.pow(
                        2, idx)), out_planes // int(math.pow(2, idx + 1)), 3))
            else:
                self.conv_list.append(
                    SeparableConvBNReLU(out_planes // int(math.pow(2, idx)),
                                        out_planes // int(math.pow(2, idx)), 3))
...
def LiteSTDC2(**kwargs):
    model = LiteSTDCNet(base=64, layers=[4, 5, 3], **kwargs)
    return model


@manager.BACKBONES.add_component
def LiteSTDC1(**kwargs):
    model = LiteSTDCNet(base=64, layers=[2, 2, 2], **kwargs)
    return model
